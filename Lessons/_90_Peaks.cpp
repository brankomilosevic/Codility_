...
int check_division(vector<int> peeks, int k)
{
    int len = peeks.size();
    if (len % k) return 0;
    int segment = len / k;
...
int solution(vector<int> &A)
{
    ...
    // ~70% grade, this one is N**2
    // for (int i=1; i<len; ++i)
    // {
    //     if (check_division(peeks, i)) 
    //         res = (res > i) ? res : i;
    // }

    // I will try with factors only and look at it in bisection mode
    // 90% grade, all performance test passed, one accuracy test failed, no-one knows which one
    vector<int> factors = generate_factors(len);
    int left = 0;
    int right = factors.size();
    int mid = (right - left) / 2;
    while (left < right)
    {
        if (check_division(peeks, factors[mid]) && !check_division(peeks, factors[mid+1])) 
            return factors[mid];

        if (check_division(peeks, factors[mid]))
            left = mid;
        else   
            right = mid;
...
int main(void)
{
    ...
    vector<int> A = {5};
    cout << "Max number of blocks: " << solution(A);
