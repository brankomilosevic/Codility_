...
B = 0
C = 0
the function should return 1073741824
...
{
    int n = 0;
    unsigned XX = ~X;    // or ~X if number of zeroes are needed
    for (int i = 0; i < 30; ++i)
        if ( XX & (1 << i) ) ++n;
    return n;
}
...
{
    int res = 0;
    int dig_A = num_of_zeroes(A);
    int dig_B = num_of_zeroes(B);
    int dig_C = num_of_zeroes(C);
    // std::cout << "\n-> A zeroes: " << dig_A;
    // std::cout << "\n-> B zeroes: " << dig_B;
    // std::cout << "\n-> C zeroes: " << dig_C;
    ...
    int dig_AB = num_of_zeroes(A | B);
    int dig_AC = num_of_zeroes(A | C);
    int dig_BC = num_of_zeroes(B | C);
...
int main(void)
{
    ...
    std::cout << "\nNumber of conforming integers is: " << solution(A, B, C);
